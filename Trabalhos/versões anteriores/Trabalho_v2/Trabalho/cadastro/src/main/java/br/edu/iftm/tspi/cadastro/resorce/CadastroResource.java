...
public class CadastroResource {
    ...
    // Alterações do dia 20/09
    @RequestMapping("/editar/{index}")
    public String editarCadastro(@PathVariable("index") int index, Model model) {
        CadastroDTO cadastro = cadastros.get(index);
        model.addAttribute("cadastro", cadastro);
        return "editar";
    }

    @PostMapping("/atualizar/{index}")
    public String atualizarCadastro(@PathVariable("index") int index, CadastroDTO cadastroAtualizado, Model model) {
        cadastros.set(index, cadastroAtualizado);
        return "redirect:/lista"; // Redireciona para a lista após a atualização.
    }

    @PostMapping("/excluir/{index}")
    public String excluirCadastro(@PathVariable("index") int index) {
